...
**Goal:** Reorganize the Playwright suite around the refined alpha-scope browser journeys and add missing coverage for RFQ creation, project detail, tasks, and settings/users without expanding into deferred routes.

**Architecture:** Keep the existing auth and CORS bootstrap helpers, then split browser coverage into one spec family per alpha journey group. The RFQ alpha journeys become the main ownership boundary for procurement flows, projects/tasks get their own small spec, and settings/users stay separate from the dashboard shell assertions. The existing smoke and lifecycle files should be trimmed so they no longer duplicate journey ownership.
...
- `apps/atomy-q/WEB/tests/projects-tasks-alpha.spec.ts`: new owner for project list/detail and task inbox/drawer.
- `apps/atomy-q/WEB/tests/screen-smoke.spec.ts`: trim any project/task overlap after the new projects/tasks spec lands.
- `apps/atomy-q/WEB/tests/settings-users-alpha.spec.ts`: new owner for settings landing and Users & Roles.
...
- `stubAlphaUsersRoles(page)` if the settings/users spec needs a reusable route bootstrap for `/api/v1/users` and `/api/v1/roles`.

Keep the helper small. Do not move RFQ/project/task fixtures into this file; those should stay local to the journey family that owns them.
...
## Task 3: Add the projects/tasks alpha spec and move the browser-visible work flows into it

**Files:**
- Create: `apps/atomy-q/WEB/tests/projects-tasks-alpha.spec.ts`
- Modify: `apps/atomy-q/WEB/tests/screen-smoke.spec.ts`
- Modify: `apps/atomy-q/WEB/tests/smoke.spec.ts`

- [ ] **Step 1: Write the new projects/tasks alpha journey tests**

Create `apps/atomy-q/WEB/tests/projects-tasks-alpha.spec.ts` with dedicated coverage for:
...
Trim `smoke.spec.ts` and `screen-smoke.spec.ts` so they no longer own the browser-visible project and task journeys once `projects-tasks-alpha.spec.ts` is in place.
...
```bash
cd apps/atomy-q/WEB
npx playwright test tests/projects-tasks-alpha.spec.ts
```
...
```bash
git add apps/atomy-q/WEB/tests/projects-tasks-alpha.spec.ts apps/atomy-q/WEB/tests/screen-smoke.spec.ts apps/atomy-q/WEB/tests/smoke.spec.ts
git commit -m "test(web): add alpha projects and tasks e2e coverage"
```

## Task 4: Add the settings/users alpha spec and reduce dashboard-nav to shell-level assertions

**Files:**
- Create: `apps/atomy-q/WEB/tests/settings-users-alpha.spec.ts`
- Modify: `apps/atomy-q/WEB/tests/dashboard-nav.spec.ts`

- [ ] **Step 1: Create the settings/users alpha spec**

Create `apps/atomy-q/WEB/tests/settings-users-alpha.spec.ts` to own the alpha-visible settings path.
...
- the link or entry point that takes the user into settings

The page-content assertions for `/settings/users` should live only in `settings-users-alpha.spec.ts`.
...
```bash
cd apps/atomy-q/WEB
npx playwright test tests/settings-users-alpha.spec.ts tests/dashboard-nav.spec.ts
```
...
```bash
git add apps/atomy-q/WEB/tests/settings-users-alpha.spec.ts apps/atomy-q/WEB/tests/dashboard-nav.spec.ts
git commit -m "test(web): add alpha settings users coverage"
```
...
- which files now own RFQ, projects/tasks, and settings/users coverage
...
- Projects/tasks and settings/users each have a dedicated alpha spec.
