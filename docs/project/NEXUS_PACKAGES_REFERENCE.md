...
| `Nexus\Vendor` | Vendor master domain package. | Vendor entities, status transitions, split persistence/query contracts, and vendor master value objects. | Not procurement transaction flow; use `Procurement` or `Sourcing`. | You need supplier/vendor master records. |
