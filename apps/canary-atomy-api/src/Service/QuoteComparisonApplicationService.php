...
final readonly class QuoteComparisonApplicationService
{
    ...
    public function compare(string $tenantId, QuoteComparisonRequestDto $request): array
    {
        ...
        $runId = (string)\Symfony\Component\Uid\Ulid::generate();
        ...
        // We use a custom constructor that accepts the ID to ensure atomicity and consistency
        // But since QuoteComparisonRun generates its own ULID in constructor, we should either:
        // 1. Update QuoteComparisonRun to accept ID (optional)
        // 2. Or just use the one generated by constructor.
        // The PR review says: "construct the final response array before creating the QuoteComparisonRun and pass that final responsePayload into the QuoteComparisonRun constructor"
        ...
        // Correcting run_id in finalResponse to match the one generated in constructor
        $finalResponse['run_id'] = $run->getId();
