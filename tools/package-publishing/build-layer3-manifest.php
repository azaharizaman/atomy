...
$adaptersDir = $root . '/adapters';
$outputDir = $root . '/docs/package-publishing';
$jsonOutput = $outputDir . '/nexus-layer3-packages.manifest.json';
$markdownOutput = $outputDir . '/nexus-layer3-packages.manifest.md';
$packagistVendor = getenv('PACKAGIST_VENDOR') ?: 'azaharizaman';
$githubOwner = getenv('GITHUB_OWNER') ?: 'azaharizaman';

if (! is_dir($adaptersDir)) {
    fwrite(STDERR, "Missing adapters directory: {$adaptersDir}\n");
    exit(1);
}
...
foreach (glob($adaptersDir . '/*/*/composer.json') ?: [] as $composerPath) {
