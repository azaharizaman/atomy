...
use Nexus\Telemetry\Exceptions\UnsupportedAggregationException;
...
use Nexus\Telemetry\Contracts\CardinalityStorageInterface;
...
use Nexus\Telemetry\Contracts\AlertDispatcherInterface;
use Nexus\Telemetry\Contracts\CacheRepositoryInterface;
use Nexus\Telemetry\Services\TelemetryTracker;
use Nexus\Telemetry\Services\HealthCheckRunner;
use Nexus\Telemetry\Services\AlertEvaluator;
...
final class RunHealthChecksCommand extends Command
{
    ...
    public function handle(HealthCheckRunner $runner): int
    {
        ...
        return $overallHealth === \Nexus\Telemetry\ValueObjects\HealthStatus::HEALTHY ? 0 : 1;
    }
}
```
...
services:
    ...
    # TelemetryTracker
    Nexus\Telemetry\Contracts\TelemetryTrackerInterface:
        class: Nexus\Telemetry\Services\TelemetryTracker
        arguments:
            $storage: '@Nexus\Telemetry\Contracts\MetricStorageInterface'
            $logger: '@logger'
            $tenantContext: '@Nexus\Tenant\Contracts\TenantContextInterface'

    # Health Check Runner
    Nexus\Telemetry\Services\HealthCheckRunner:
        arguments:
            $logger: '@logger'
            $cache: '@cache.app'

    # Alert Evaluator
    Nexus\Telemetry\Services\AlertEvaluator:
        arguments:
