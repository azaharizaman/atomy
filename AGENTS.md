...
- **Date:** 2026-04-22
- **PR/Issue ID:** [#378](https://github.com/azaharizaman/atomy/pull/378)
- **Summary of mistake:** Vendor governance follow-up review found a cluster of boundary issues: governance lookups used `lower(...)` predicates without guaranteeing canonical stored identifiers, due-diligence PATCH nulled omitted `reviewed_by`, sanctions screening created evidence without the project’s idempotency completion flow, vendor approval metadata still mixed nullable design intent with empty-string schema/seed values, and the governance docs/tests were not fully synced to the new tables/models and deterministic scores.
- **Root cause:** I closed the feature slice around primary workflow behavior and missed a final pass for query sargability assumptions, PATCH field-presence semantics, persisted POST idempotency parity, and inventory/documentation consistency after adding new governance persistence.
- **Action taken:** Canonicalized vendor/governance identifiers on write and switched governance reads to equality predicates, preserved omitted `reviewed_by` values in due-diligence PATCH handling, added idempotency middleware plus `IdempotencyCompletion` to sanctions screening, made `approved_by_user_id` nullable in the vendor adapter migration and petrochemical seed data, tightened governance score/API/frontend tests, and updated the API implementation summary inventory and counts.
- **Follow-up tasks:** Before resolving future sourcing/vendor review threads, require a closure checklist for (1) canonical identifier storage before replacing case-insensitive predicates, (2) field-presence handling on PATCH APIs, (3) middleware-plus-completion wiring for every persisted POST route, and (4) doc/schema/test inventory updates whenever new tables or models are introduced.

---

- **Date:** 2026-04-24
- **PR/Issue ID:** [#381](https://github.com/azaharizaman/atomy/pull/381)
- **Summary of mistake:** AI quote-intake continuity shipped with several reviewable contract and concurrency gaps: provider AI transport used generic exceptions and concrete dependencies, manual source-line POST lacked idempotency, decision-trail writes were duplicated in the controller without canonical locking, manual source-line mutations were not atomic, upload briefly persisted a transient `uploaded` state before manual fallback, manual edits clobbered existing `raw_data`, the frontend did not pass RFQ-line mapping fields for manual lines, and provider DTO validation allowed decimal line numbers to be truncated.
- **Root cause:** I focused on making the provider/manual fallback path work end to end and did not complete a final pass for interface-first adapter boundaries, POST retry semantics, transaction scope around multi-write flows, UI-to-request payload parity, and strict provider payload normalization.
- **Action taken:** Added provider transport contracts and domain exceptions, rejected list-style JSON responses, bound the transport interface in the container, moved manual decision-trail events through `DecisionTrailRecorder` with locked chained writes, wrapped manual source-line create/update/delete flows in transactions, added idempotency middleware/completion plus frontend idempotency key coverage for manual source-line POST, removed duplicate raw provenance from normalization responses, preserved existing `raw_data` while updating manual provenance, added RFQ-line selection/note payload support and duplicate-submit guards in the normalize UI, tightened ProcurementML payload validation, and updated tests/docs.
- **Follow-up tasks:** Before future AI quote-intake PRs, require a review checklist for (1) interface-first provider adapter dependencies, (2) domain exception mapping for provider/runtime failures, (3) idempotency completion on every persisted POST, (4) transaction boundaries across source-line/readiness/audit writes, (5) frontend payload parity for readiness-critical fields, and (6) strict provider DTO normalization with no lossy casts.

---

- **Date:** 2026-04-24
- **PR/Issue ID:** Plan 4 CodeRabbit review follow-up
- **Summary of mistake:** The first Plan 4 pass left several quality gaps: comparison/award transport calls were not operation-disambiguated, approval summary fetched unscoped lazy relations in one endpoint path, tests carried dead helper code and weak payload assertions, and the award AI panel surfaced an "AI-derived" badge before wiring real guidance payload rendering.
- **Root cause:** I focused on landing functional endpoint and UI scaffolding quickly, then deferred a final review for relation-scoping parity, dead test helpers, stricter test assertions, and “ready vs placeholder” UX semantics.
- **Action taken:** Added an explicit `action` discriminator to comparison/award/approval AI transport payloads, tenant-scoped eager loading in approval summary, narrowed comparison overlay exception handling, removed dead feature-test helpers, strengthened approval summary payload assertions, wired award-page guidance panel to real guidance payload data, and synchronized implementation summary coverage entries.
- **Follow-up tasks:** Before merging future AI surface slices, run a mandatory pass for (1) operation-disambiguated provider payloads on shared endpoints, (2) tenant-scoped eager loading parity across all controller paths that serialize relations, (3) dead-helper cleanup in tests, and (4) no “AI-derived” UI indicators unless live AI payload data is actually displayed.

---

- **Date:** 2026-04-24
- **PR/Issue ID:** Plan 4 review hardening follow-up
- **Summary of mistake:** The follow-up review still exposed contract and defensive-coding drift across the comparison/award/approval intelligence slice: the shared AI client still used weak array signatures, a few controllers still mixed in service location or ad hoc timestamp generation, persisted artifact readers did not fully guard empty identifier segments, and several WEB consumers silently coerced invalid AI payloads or allowed live queries in mock mode.
- **Root cause:** I closed the functional slice before running a stricter contract-hardening pass across adapter boundaries, controller dependency style, artifact lookup guards, and frontend payload normalization semantics.
- **Action taken:** Replaced the shared AI client array contract with typed request/response DTOs plus domain exception docs, kept provider transport failures wrapped at the adapter boundary, completed constructor-injected runtime/clock usage in the remaining controllers, documented and guarded artifact metadata/lookups in the decision trail, documented the persisted `summary_payload` schema, moved the pure approval-summary normalization assertion into a dedicated hook test, and tightened WEB hooks/pages to reject invalid boolean states, disable live fetches in mock mode, memoize unstable derived arrays, and consolidate duplicated unavailable branches.
- **Follow-up tasks:** Before closing future AI review rounds, require an explicit pass for (1) typed DTO boundaries on shared adapters, (2) no `app(...)` service location where constructor DI is viable, (3) documented JSON schema notes for newly persisted artifact payloads, (4) strict frontend normalizer validation instead of boolean coercion, and (5) mock-mode gating on every live query hook.

---

- **Date:** 2026-04-24
- **PR/Issue ID:** Comparison/award/approval intelligence review follow-up
- **Summary of mistake:** A later review still found trimmed-but-uncanonicalized DTO identifiers, unchecked AI mode values, exception messages that could drop context or leak full payloads, GET endpoints that still generated and persisted AI artifacts, unlocked artifact read-modify-write paths, and WEB AI views/normalizers that were too permissive or too raw for live operator use.
...
- **Date:** 2026-04-24
- **PR/Issue ID:** Plan 6 CodeRabbit review follow-up
- **Summary of mistake:** The launch-readiness hardening pass still had a few avoidable operational hygiene gaps: alert recipient/notification value objects accepted empty identifiers, transport and console error paths exposed raw exception messages, the scheduled AI status publisher could overlap across nodes, and I initially used an invalid normalization pattern on readonly promoted properties.
- **Root cause:** I prioritized getting the new operational runtime in place and deferred a final pass for PHP 8.3 readonly initialization semantics, sanitized operator-visible diagnostics, scheduler concurrency safety, and shared-test-state cleanup.
- **Action taken:** Added strict non-empty validation plus deterministic normalization for AI alert recipient/notification objects, sanitized console and transport error reporting, bounded retry/backoff values, added `withoutOverlapping()->onOneServer()` to the scheduled AI status command, removed the shared static provider-spy counter from tests, and tightened the launch-readiness docs to reference exact plan/spec locations.
- **Follow-up tasks:** Before merging future operational tooling, explicitly check (1) readonly constructor normalization patterns, (2) no raw exception messages in console output or structured logs, (3) scheduler overlap and multi-node behavior, and (4) hidden shared mutable state in test helpers.

---

- **Date:** 2026-04-25
- **PR/Issue ID:** AI launch readiness review follow-up
- **Summary of mistake:** The post-launch-readiness review still exposed several boundary and verification gaps: transport retry defaults/documentation were not fully aligned with runtime behavior, alert publishing lacked a recovery/clear path and still relied on concrete dependencies, outbox wiring was effectively inert in the app container, and the RFQ vendors page test realignment drifted from Next App Router’s promised-params contract.
- **Root cause:** I focused first on getting the AI operational surface working end to end, then underweighted a final pass for interface-first DI, cooldown/recovery lifecycle semantics, container completeness, and test harness fidelity to framework-generated types.
- **Action taken:** Hardened `ProviderAiTransport` retry/backoff/log severity and unsupported-endpoint handling, added provider-client interfaces plus `AiOperationalAlertPublisherInterface`, bound outbox services explicitly, added alert-cleared/cooldown tests with isolated cache state, sanitized operator-facing diagnostics and recipient identifiers, documented total-attempt semantics in config/env, updated dated implementation headings, and realigned the RFQ vendors tests to render the page through `Suspense` with promised params while keeping `npm run build` green.
- **Follow-up tasks:** Before closing future AI/platform review threads, explicitly verify (1) retry semantics wording matches the actual loop behavior, (2) alert flows cover degrade, suppress, and clear transitions, (3) console/runtime services depend on interfaces where practical, (4) cache/time-sensitive tests isolate their state, and (5) App Router page tests mirror Next’s generated `PageProps` contract instead of using convenience-only prop shapes.
