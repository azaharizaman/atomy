...
$orchestratorsDir = $root . '/orchestrators';
$outputDir = $root . '/docs/package-publishing';
$jsonOutput = $outputDir . '/nexus-layer2-packages.manifest.json';
$markdownOutput = $outputDir . '/nexus-layer2-packages.manifest.md';
$packagistVendor = getenv('PACKAGIST_VENDOR') ?: 'azaharizaman';
$githubOwner = getenv('GITHUB_OWNER') ?: 'azaharizaman';

if (! is_dir($orchestratorsDir)) {
    fwrite(STDERR, "Missing orchestrators directory: {$orchestratorsDir}\n");
    exit(1);
}
...
foreach (glob($orchestratorsDir . '/*/composer.json') ?: [] as $composerPath) {
...
$markdown = [
    '# Nexus Layer 2 Packagist Publishing Manifest',
    '',
