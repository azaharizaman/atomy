...
| `normalization_intelligence` | Staging provider contract test passes, provider-mode normalization closes end-to-end, and degraded manual continuity is proven | AI suggestions work with real provider; at least one audited override is captured; readiness clears and comparison freeze succeeds; manual mapping and overrides still work when provider normalization degrades | Suggestions fail without truthful state, audited override evidence is missing, comparison freeze cannot close after normalization, or manual normalize path is blocked |
