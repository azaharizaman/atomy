...
- [ ] **Step 1:** Write failing feature test: authenticated user A receives activity for own RFQ; user B (other tenant) gets **404** for same id.

```php
// apps/atomy-q/API/tests/Feature/RfqOverviewActivityTest.php
public function test_activity_is_tenant_scoped(): void
{
    // Arrange: rfq id belonging to tenant A
    // Act: GET /api/v1/rfqs/{id}/activity as tenant B
    // Assert: 404
    $this->markTestIncomplete('Implement after route exists');
}
```

- [ ] **Step 2:** Implement `GET /api/v1/rfqs/{rfqId}/activity?limit=20` (or embedded in overview if product insists) with tenant-scoped query.

- [ ] **Step 3:** Add overview fields (`expectedQuotes`, `normalizationProgress`, comparison summary, `approvalStatus`) **as far as data exists**; use `null` or omit with OpenAPI description for not-yet-implemented.

- [ ] **Step 4:** `cd apps/atomy-q/API && php artisan test --filter=RfqOverviewActivity` — expect PASS.
